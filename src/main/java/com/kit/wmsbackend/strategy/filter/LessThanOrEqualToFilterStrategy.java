...
public class LessThanOrEqualToFilterStrategy<T> implements FilterStrategy<T> {
    ...
    public Predicate apply(Root<T> root, CriteriaBuilder cb, Path<?> path, Object value) {
        ...
        if (!(normalizedValue instanceof Comparable<?> comparableValue)) {
            throw new AppException(ErrorCode.FILTER_INVALID_VALUE, "Value for 'lt or eq' filter must implement Comparable");
        }

        return cb.lessThanOrEqualTo(resolvedPath.as(comparableValue.getClass()), (Comparable) comparableValue);
    }
}